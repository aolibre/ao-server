...
Public Function PuedeCaminarOculto(UserIndex As Integer) As Boolean
    With UserList(UserIndex)
        PuedeCaminarOculto = False
        If .Clase = eClass.Hunter And .Stats.UserSkills(eSkill.Ocultarse) > 99 Then
            If .Invent.ArmourEqpObjIndex = 648 Or .Invent.ArmourEqpObjIndex = 360 Then
                PuedeCaminarOculto = True
                Exit Function
            End If
        End If
        If .Clase = eClass.Thief Or .Clase = eClass.Bandit And .Stats.UserSkills(eSkill.Ocultarse) = 100 Then
            PuedeCaminarOculto = True
        End If
    End With
End Function
